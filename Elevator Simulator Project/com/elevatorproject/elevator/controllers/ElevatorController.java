...
public class ElevatorController implements com.nodalexchange.elevator.ElevatorController{
	...
	public void cycleElapsed(Building building) {
		...
		for(int i=0;i<building.getNumberOfElevators();i++) { 		// for loops iterates through every elevator
			...
			if(building.getElevator(i).getCurrentFloor()!=0 && building.getElevator(i).getCurrentFloor()!=building.getNumberOfFloors()-1 && building.getElevator(i).getButtonToAnswerAtNextStop()==DirectionEnum.UP) {
			...
			else if(building.getElevator(i).getCurrentFloor()!=0 && building.getElevator(i).getCurrentFloor()!=building.getNumberOfFloors()-1 && building.getElevator(i).getButtonToAnswerAtNextStop()==DirectionEnum.DOWN) {
			...
			public boolean checkGoingUp(Building building,int number) {
				//this function checks for all passengers in elevator going up first and if no passengers it checks all the lobbies for passengers going up returns false if no passenger going up exists
				...
				building.getElevator(number).setNewInstruction(nextStop, building.getElevator(number).getButtonToAnswerAtNextStop().UP);
				return flag1;
			...
			public void checkGoingDowns(Building building,int number) {
				//this function checks for all passengers in elevator going down first and if no passengers it checks all the lobbies for passengers going down 
				...
				if(flag) {
				 ...
				 nextStop=p;
				 trust1=true;
				building.getElevator(number).setNewInstruction(nextStop, building.getElevator(number).getButtonToAnswerAtNextStop().DOWN);
				...
				if(trust1==false) {
					for(int j=building.getNumberOfFloors()-1;j>=0;j--) {
						...
						if(building.getElevatorWaitingAreaInfo(j).isDownButtonPushed()) {
							building.getElevator(number).setNewInstruction(j, building.getElevator(number).getButtonToAnswerAtNextStop().DOWN);
							trust1=true;
							break;	
							}
					}
			...
			public void checkGoingUps(Building building,int number) {
				//this function checks for all passengers in elevator going up first and if no passengers it checks all the lobbies for passengers going up
			...
			public boolean checkGoingDown(Building building,int number) {
				//this function checks for all passengers in elevator going down first and if no passengers it checks all the lobbies for passengers going down returns false if no passenger going down exists
